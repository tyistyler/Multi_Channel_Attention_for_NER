...
from utils_token_level_task import processors
...
def load_examples(data_dir, max_seq_len, tokenizer, processor, label_list, mode):
    if mode == "train":
        data_dir = os.path.join(data_dir, "train.txt")
        examples = processor.load_tsv_data(data_dir, flag=mode)
    elif mode == "dev":
        data_dir = os.path.join(data_dir, "dev.txt")
        examples = processor.load_tsv_data(data_dir, flag=mode)
    elif mode == "test":
        data_dir = os.path.join(data_dir, "test.txt")
        examples = processor.load_tsv_data(data_dir, flag=mode)

    features = processor.convert_ngram_examples_to_features(examples, label_list, max_seq_len, tokenizer)
    # all_ntokens = [f.ntokens for f in features]
    '''
    input_ids=input_ids, input_mask=input_mask, segment_ids=segment_ids, label_id=label_ids, valid_ids=valid,
    label_mask=label_mask, word_ids=word_ids, matching_matrix=matching_matrix, channel_ids=channel_ids,
    ...
    all_matching_matrix = torch.tensor([f.matching_matrix for f in features], dtype=torch.long)
    all_channel_ids = torch.tensor([f.channel_ids for f in features], dtype=torch.long)

    return TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_label_ids, all_word_ids, all_matching_matrix,
