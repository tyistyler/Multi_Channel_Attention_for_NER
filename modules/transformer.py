...
class BertSelfAttention(nn.Module):
    def __init__(self, hidden_size=768, attention_probs_dropout_prob=0.1, num_attention_heads=4, output_attentions=False, keep_multihead_output=False):
...
class BertSelfOutput(nn.Module):
    def __init__(self, hidden_size=768, dropout_rate=0.1, layer_norm_eps=1e-12):
...
class BertOutput(nn.Module):
    def __init__(self, intermediate_size=3072, hidden_size=768, dropout_rate=0.1, layer_norm_eps=1e-12):
...
class AdaptedTransformerEncoder(nn.Module):
    def __init__(self, num_layers, d_model, n_head, feedforward_dim, dropout, ngram_embed_size=768, after_norm=True, attn_type='naive',
        ...
        layer = BertLayer(hidden_size=ngram_embed_size,output_attentions=None, keep_multihead_output=None)
        ...
        # self.convert = nn.Linear(ngram_embed_size, d_model)
    ...
    def forward(self, x, mask, ngram_hidden_states=None, ngram_attention_mask=None, ngram_position_matrix=None):
        ...
        for i, layer in enumerate(self.layers):
            ...
            # w_hidden = self.convert(w_hidden)
            x += w_hidden
        return x
